...
namespace sensor_neighbor {
  ...
  void SensorNeighbor::callbackNeighborsUsingGPS(const nav_msgs::Odometry::ConstPtr& odom, const unsigned uav_id) {
    if (!is_initialized_) {
      return;
    }

    {
      ...
      if (neighbors_position_.find(uav_id) == neighbors_position_.end()) {
        neighbors_position_.insert(std::pair<unsigned, geometry_msgs::PointStamped>(uav_id, uav_point));
      }else {
...

    {
      ...
      for (unsigned int i = 0; i < array_poses->poses.size(); i++) {
        ...
        if (uav_point_transformed.has_value()) {
            ...
            if (neighbors_position_.find(uav_id) == neighbors_position_.end()) {
              neighbors_position_.insert(std::pair<unsigned, geometry_msgs::PointStamped>(uav_id, uav_point_transformed.value()));
            }else {
              neighbors_position_[uav_id] = uav_point_transformed.value();
            }
        } else {
            ROS_WARN("Could not transform pose from /stable_origin to /fcu!");
        }
      }
    }
  }
